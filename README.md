...
Servidor Model Context Protocol (MCP) para la integración con **Factuplan**.
...
-   APIs especializadas para emisión de facturas electrónicas mediante el proveedor Factuplan.
...
-   `emitir_factura`: Genera el comprobante usando la plataforma Factuplan.
...
```env
FACTUPLAN_API_KEY="tu_api_key_aqui"
```
...
docker run -d \
  --name mcp-factuplan \
  --env-file .env \
  ghcr.io/mcphub-ec/mcp-factuplan:latest
```
...
services:
  mcp-factuplan:
    image: ghcr.io/mcphub-ec/mcp-factuplan:latest
    container_name: mcp-factuplan
    env_file:
      - .env
